...
class EnvironmentHandler:
    ...
    ANNCSU_CLI_MODULE = "anncsu_toolkit.cli"
...
class DockerEnvironmentHandler(EnvironmentHandler):
    ...
    Makes some assumptions, for example that the image includes Poetry env with ANNCSU Toolkit in it."""
    ...
    def verify_toolkit(self, required_version: str, env) -> Tuple[bool, str]:
        """
        Verifies if ANNCSU Toolkit is installed and checks if the installed version matches the required version.
        
        Args:
            required_version: The version of ANNCSU Toolkit required by the plugin.
        ...
        try:
            cmd = [
                self.docker_path, "run", "--rm", self.image_name, "poetry", "run", "python", "-c", "import anncsu_toolkit"
            ]
            ...
            if result.returncode != 0:
                return False, f"ANNCSU Toolkit is not installed in the Docker image '{self.image_name}."
            
            return True, f"ANNCSU Toolkit is installed in the Docker image '{self.image_name}'."

            # Parse the output to extract the version of ANNCSU Toolkit
            # version = None
            ...
            # if version is None:
            #     return False, "ANNCSU Toolkit version information could not be retrieved."

            # if version != required_version:
            #     return (
            #         False,
            #         f"ANNCSU Toolkit version {version} is installed, but version {required_version} is required."
            #     )

            # return True, f"ANNCSU Toolkit version {version} is correctly installed."

        except subprocess.CalledProcessError as e:
            return False, f"ANNCSU Toolkit is not installed in the Docker image '{self.image_name}'. Error: {e.stderr}"


    # def upgrade_toolkit(self, env) -> Tuple[bool, str]:
    #     """
    #     Upgrades ANNCSU Toolkit to the latest version in the specified Docker image.
    ...
    #             "pip", "install", "anncsu_toolkit", "--upgrade"
    #         ]
    #         result = subprocess.run(
    #             cmd,
    #             stdout=subprocess.PIPE,
    ...
    #         if result.returncode == 0:
    #             return True, "ANNCSU Toolkit was successfully upgraded in the Docker image."
    #         else:
    #             return False, f"Failed to upgrade ANNCSU Toolkit in the Docker image: {result.stderr}"

    #     except subprocess.CalledProcessError as e:
    #         return False, f"Failed to upgrade ANNCSU Toolkit in the Docker image: {e.stderr}"
...
class VenvEnvironmentHandler(EnvironmentHandler):
    ...
    def verify_toolkit(self, required_version: str, env) -> Tuple[bool, str]:
        ...
        """
        try:
            cmd = [self.python_path, "-m", "pip", "show", "anncsu_toolkit"]
            creationflags = 0
            ...
            if result.returncode != 0:
                return False, "ANNCSU Toolkit is not installed in the specified venv."
            ...
            return False, f"ANNCSU Toolkit is not installed in the specified venv. Error: {e.stderr}"


    def upgrade_toolkit(self, env) -> Tuple[bool, str]:
        """
        Upgrades ANNCSU Toolkit to the latest version in the specified virtual environment.
        ...
        """
        try:
            cmd = [self.python_path, "-m", "pip", "install", "anncsu_toolkit", "--upgrade"]
            creationflags = 0
            if os.name == 'nt':  # If Windows, prevent process window creation
                creationflags = subprocess.CREATE_NO_WINDOW
            result = subprocess.run(
            ...
            if result.returncode == 0:
                return True, "ANNCSU Toolkit was successfully upgraded."
            else:
                return False, f"Failed to upgrade ANNCSU Toolkit: {result.stderr}"

        except subprocess.CalledProcessError as e:
            return False, f"Failed to upgrade ANNCSU Toolkit: {e.stderr}"
