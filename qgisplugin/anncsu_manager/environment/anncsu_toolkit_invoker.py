...
from anncsu_manager.environment.anncsu_environment_handler import DockerEnvironmentHandler, VenvEnvironmentHandler
from anncsu_manager.utils.model_feedback import ANNCSUProcessingFeedback
from anncsu_manager.utils.settings_manager import ANNCSUSettingsManager

DEBUG = True
REQUIRED_ANNCSU_TOOLKIT_VERSION = "1.1.6"
...
class ANNCSUToolkitInvoker:
    """Class that handles communication between ANNCSU QGIS plugin and ANNCSU Toolkit."""
    ...
    def __init__(self, env_type = None, venv_directory = None, docker_path = None, docker_image_name = None):
        """Initializes the ANNCSUToolkitInvoker."""
        env_type = ANNCSUSettingsManager.get_environment_selection() if env_type is None else env_type

        # Environment handler is needed for environment-specific communication
        if env_type == "venv":
            self.environment_handler = VenvEnvironmentHandler(
                ANNCSUSettingsManager.get_venv_directory() if venv_directory is None else venv_directory
            )
        elif env_type == "docker":
            self.environment_handler = DockerEnvironmentHandler(
                ANNCSUSettingsManager.get_docker_path() if docker_path is None else docker_path,
                ANNCSUSettingsManager.get_docker_image_name() if docker_image_name is None else docker_image_name,
                ANNCSUSettingsManager.get_docker_host_folder(),
                ANNCSUSettingsManager.get_docker_temp_folder()
            )
        else:
    ...
    def assemble_cli_command(self, alg_name: str, typer_args: List[str], typer_options: List[str]):
        """
        Assembles command-line interface command for a specific algorithm of ANNCSU toolkit.
        ...
        if isinstance(self.environment_handler, VenvEnvironmentHandler) and not self.environment_handler.venv_directory:
            raise EnvironmentException(
                "ANNCSU Toolkit environment has not been set, cannot execute ANNCSU tools! Configure ANNCSU Toolkit " +
                "in Settings page of ANNCSU Wizard.")
    ...
    def verify_toolkit(self) -> Tuple[bool, str]:
        """Checks if ANNCSU Toolkit can be found in the selected environment."""
        return self.environment_handler.verify_toolkit(REQUIRED_ANNCSU_TOOLKIT_VERSION, self.python_free_environment)
    ...
    def run_toolkit_command(self, feedback: QgsProcessingFeedback) -> dict:
        ...
        # Execute ANNCSU Toolkit through subprocess
        try:
            ...
            feedback.pushInfo("[OPENING ANNCSU TOOLKIT]\n")
            ...
            if self.process.returncode != 0:
                feedback.reportError("ANNCSU Toolkit algorithm execution failed.")
            # else:
            #     feedback.pushInfo("ANNCSU Toolkit algorithm executed successfully!")

        except TerminationException as e:
            if type(feedback) == ANNCSUProcessingFeedback:
        ...
        # Handle potential exceptions
            ...
            feedback.pushInfo("[CLOSING ANNCSU TOOLKIT]\n")
