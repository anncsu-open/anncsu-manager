...
- Layer creation task no longer holds a reference to the `QgsVectorLayer` inside the `QgsTask` object, which was causing QGIS to hang on Windows 11; the layer is now cloned before being returned and the task is explicitly deleted to free memory (#16)
...
- Added missing Italian translations for 6 untranslated strings across wizard settings, geocoder step, and geocode task contexts
